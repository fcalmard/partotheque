	...
	public function ChargeListe(array $aFiltre = null,$aTriOeuvresSession=null,$bAffAppPdf=false)
	{
				...
				foreach ($aTri as $koTri=>$oTri)
				{
					...
					if($oTri!="")
					{
						...
						if(strtolower($koTri)=="o.reference")
						{
							...
							//$koTri="CAST(".$koTri." AS DECIMAL(5,2))";
							//$koTri="LDAP(".$koTri." ,5,'0' )";
							//$koTri="val(".$koTri.")";
							...
							//echo "TRI PAR REFERENCE >".$sTri."<";
							$ssenstri=$oTri;
							if($ssenstri=='desc')
							{
								
							}
							//echo " ssenstri>".$ssenstri."<";
		...
		try {
			...
			if($btriparref)
			{
				...
				$orderby = "reference"; //change this to whatever key you want from the array
				//
				$sortArray = array();
				//
				foreach($a as $oe){
				...
				foreach ($a as $oe)
				{
					...
					$ixref=intval($oe['id']);
					$iref=intval($oe['reference']);
					
					//$asortarray[$oe['id']]=array('id'=>$ixref,'reference'=>$iref);
					$asortarray[$ixref]=array('reference'=>$iref);
					
					if($ssenstri=='desc')
					{
						//echo "<br/> ID=".$ixref.' '.$oe['reference'];
						//var_dump($oe);
						//die('478');
					}
				...
				{
					asort($asortarray);			
					array_multisort($a,$asortarray);
					
				}else 
				{
					//asort($asortarray);
					//$arr = array_reverse($arr, true);
					//$asortarray= array_reverse($asortarray, true);
					arsort($asortarray);
					//$a=$asortarray;
				...
				if($ssenstri=='desc')
				{
					...
					foreach ($asortarray as $ka=>$oe)
					{
						//echo "<br/> reference >".$ka."< ......>".$oe['reference']."<";//.$oe['id']." ".$oe['reference'];
						foreach ($a as $x=>$oev)
						{
							$ka2=$oev['id'];
							$sref2=$oev['reference'];
							if($ka2==$ka)
							{
								//echo "<br/> trouve reference >".$ka2."< REF2 >".$sref2."<br/>";
			...
			if($ssenstri=='desc')
			{
				...
				$a=$arever;
						...
						//array_multisort($sortArray[$orderby],SORT_DESC,$a); 
						//change this to whatever key you want from the array
						
						//array_multisort($sortArray[$orderby],SORT_DESC,$a);
						//array_multisort($sortArray,SORT_DESC,$a);
						//var_dump($a);
						//die('417');
						//var_dump($sortArray);
						//die('428');
						/**
						 * trier $a par reference
						 */
						//$a=array_multisort($a,array('o.reference'));
						//ksort($a);
			
			foreach ($a as $oeuvre)
			//foreach ($sortArray as $oeuvre)
				{
				//var_dump($oeuvre);
				//var_dump($oeuvre['reference']);
				//die('405');
